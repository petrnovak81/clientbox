Imports System.Data.Entity.Core.Objects
Imports System.IO
Imports System.Net
Imports System.Net.Http.Headers
Imports System.Net.Mail
Imports System.Web.Http
Imports Microsoft.AspNet.SignalR
Imports Newtonsoft.Json
Imports Syncfusion.Pdf
Imports Syncfusion.HtmlConverter
Imports System.Xml
Imports System.Xml.Xsl

Namespace Controllers
    Public Class IpInfo
        <JsonProperty("ip")>
        Public Property Ip As String
        <JsonProperty("hostname")>
        Public Property Hostname As String
        <JsonProperty("city")>
        Public Property City As String
        <JsonProperty("region")>
        Public Property Region As String
        <JsonProperty("country")>
        Public Property Country As String
        <JsonProperty("loc")>
        Public Property Loc As String
        <JsonProperty("org")>
        Public Property Org As String
        <JsonProperty("postal")>
        Public Property Postal As String
    End Class

    Public Class SafeBerryController
        Inherits ApiController

        Private Shared hub As IHubContext = GlobalHost.ConnectionManager.GetHubContext(Of AgiloHub)()

        Function lastLapse(id As Integer) As String
            Dim lastLapseText As New ObjectParameter("LastLapseText", GetType(String))
            Using db As New Data4995Entities
                db.AGsp_Get_LastLapse(id, lastLapseText)
                If Not IsDBNull(lastLapseText.Value) Then
                    Return lastLapseText.Value
                Else
                    Return "LL_LastLapse: " & id
                End If
            End Using
        End Function

        <HttpPost>
        <ValidateInput(False)>
        Public Function XamarinLogin() As Object
            Dim q = HttpContext.Current.Request.Form
            Dim email = ""
            If Not String.IsNullOrEmpty(q("email")) Then
                email = q("email")
            End If
            Dim password = ""
            If Not String.IsNullOrEmpty(q("password")) Then
                password = q("password")
            End If
            If email = "hanzl@agilo.cz" And password = "123456" Then
                Return New With {.success = "OK", .error = Nothing}
            Else
                Return New With {.success = Nothing, .error = "Nesprávné přihlašovací údaje"}
            End If
            'Using db As New Data4995Entities
            '    Dim acc = db.AG_tblUsers.FirstOrDefault(Function(e) e.UserLogin = email)
            '    If acc IsNot Nothing Then
            '        Dim hashpwd As String = GetMd5Hash(email.ToLower & password)
            '        If hashpwd = acc.UserPWD Then
            '            If acc.UserAccountEnabled Then

            '            Else
            '                'ucet neni povolen

            '            End If
            '        Else
            '            'zadal spatne heslo k uctu

            '        End If
            '    Else
            '        'ucet neexistuje

            '    End If
            'End Using
        End Function

        <HttpGet>
        Public Function XamarinClients()
            Try
                Dim qs = HttpContext.Current.Request.QueryString
                Dim rr_BackupFilterProfile = 0
                If Not String.IsNullOrEmpty(qs("rr_BackupFilterProfile")) Then
                    rr_BackupFilterProfile = qs("rr_BackupFilterProfile")
                End If
                Dim hledej = ""
                If Not String.IsNullOrEmpty(qs("hledej")) Then
                    hledej = qs("hledej")
                End If
                Using db As New Data4995Entities
                    Dim data = db.AGsp_Get_DoctorumBackupProfile_G2(rr_BackupFilterProfile, hledej).OrderByDescending(Function(e) e.DatumZalozeni).ToList
                    Return New With {.success = data, .error = Nothing}
                End Using
            Catch ex As Exception
                While ex.InnerException IsNot Nothing
                    ex = ex.InnerException
                End While
                Return New With {.success = Nothing, .error = ex.Message}
            End Try
        End Function

        <HttpGet>
        Public Function EmailBodyMesicniReport(iDBackupProfile As Integer, vaseID As String, emailTo As String) As Object
            Try
                Dim html = <div style="margin: 40px auto; display: table; color: rgba(0,0,0,0.87);font-family: Roboto,RobotoDraft,Helvetica,Arial,sans-serif; box-shadow: 0px 2px 2px 0px rgb(0 0 0 / 14%), 0px 3px 1px -2px rgb(0 0 0 / 12%), 0px 1px 5px 0px rgb(0 0 0 / 20%); border-radius: 2px; padding: 20px; ">
                               <a href="https://www.doctorum.cz/Navigace/zalohovani">
                                   <img height="40" src="data:image/png;base64,"/>
                               </a>
                               <p style="margin-left:10px;margin-top:20px;font-size:14px;">Dobrý den</p>
                               <p style="margin-left:10px;font-size:14px;">vylepšili jsme vaše automatické zálohování SafeBerry o měsíční report, abyste měli jednoduchý přehled, jaká byla úspěšnost každodenního zálohování. Tento report vám budeme posílat pravidelně začátkem každého měsíce.</p>
                               <p style="margin-left:10px;font-size:14px;">Pokud se v některé dny nepovede zálohu udělat nebo je provedena jen částečně, není to na závadu, dokončí se automaticky následující den. Může to být způsobeno okolnostmi, jako jsou např.:</p>
                               <ul style="margin-left:10px;font-size:14px;">
                                   <li>momentálně nefunkčí internetové připojení</li>
                                   <li>vypnutý počítač (dovolená, zástup)</li>
                                   <li>nevhodně nastavený čas zálohování</li>
                               </ul>
                               <p style="margin-left:10px;font-size:14px;">Provádění vašich záloh hlídáme a vícedenní výpadky okamžitě řešíme, protože to signalizuje, že je něco v nepořádku.</p>
                               <h4 style="margin-left: 10px; margin-right: 10px; margin-bottom:0; font: 400 20px/28px Roboto,RobotoDraft,Helvetica,Arial,sans-serif;">Měsíční report o provedených zálohách služby <b>Safe<span style="color:red;">Berry</span></b></h4>
                               <h4 style="margin-left: 10px; margin-right: 10px; margin-top:0; font: 400 20px/28px Roboto,RobotoDraft,Helvetica,Arial,sans-serif;">Vaše ID: #<%= vaseID %></h4>
                               <table style="border-collapse: collapse;font-size:14px;">
                                   <tr style="border-bottom: 1px solid rgba(0,0,0,0.12); color: rgba(0,0,0,0.54);"><td style="padding:10px;">Datum</td><td style="padding:10px;">Čas</td><td style="padding:10px;">Den v týdnu</td><td style="padding:10px;">Úspěšná</td><td style="padding:10px;">Velikost zálohy</td></tr>
                               </table>
                               <p style="margin-left: 10px; margin-right: 10px; color: rgba(0,0,0,0.54);font-size:14px;">
                                       O vaše data je správně postaráno...
                                    </p>
                               <div style="color:rgba(0,0,0,0.54);text-align:right;padding:10px 0 0 0;">
                                   <a style="font-size:11px;color:rgba(0,0,0,0.54);text-decoration: none;" href="https://www.doctorum.cz">© <%= Now.Year %> DOCTORUM.CZ s.r.o.</a>
                               </div>
                           </div>

                Dim table = html.<table>(0)
                Dim lL_LastLapse As New ObjectParameter("LL_LastLapse", GetType(Integer))
                Using db As New Data4995Entities
                    Dim data = db.AGsp_Get_MesicniReport(iDBackupProfile, Now, lL_LastLapse).ToList
                    If lL_LastLapse.Value > 0 Then
                        Return New With {.message = lastLapse(lL_LastLapse.Value)}
                    End If
                    For Each d In data
                        Dim den = If(d.Den IsNot Nothing, d.Den.Value.ToString("dd.MM.yyyy"), "")
                        Dim cas = If(d.BackupDate IsNot Nothing, d.BackupDate.Value.ToString("HH:mm"), "")
                        Dim size = If(d.VelikostZalohy IsNot Nothing, FileSize(d.VelikostZalohy), "")
                        Dim trstyle = "border-bottom: 1px solid rgba(0,0,0,0.12); color: rgba(0,0,0,0.54); background-color: #ffffff;"
                        If d.DenVTydnu = 7 Or d.DenVTydnu = 1 Then
                            trstyle = "border-bottom: 1px solid rgba(0,0,0,0.12); color: rgba(0,0,0,0.54); background-color: rgba(0,0,0,0.12);"
                        End If
                        table.Add(<tr style=<%= trstyle %>>
                                      <td style="padding:10px;"><%= den %></td>
                                      <td style="padding:10px;"><%= cas %></td>
                                      <td style="padding:10px;"><%= d.DVT %></td>
                                      <td style="padding:10px;"><%= d.Uspesna %></td>
                                      <td style="padding:10px;"><%= size %></td>
                                  </tr>)
                    Next
                End Using

                Dim obj = New With {
                .emailTo = emailTo,
                .emailSubject = "DOCTORUM.CZ - měsíční přehled zálohování SafeBerry",
                .emailBody = html.ToString,
                .MDate = Nothing
                }


                Return New With {.data = obj, .error = Nothing}
            Catch ex As Exception
                While ex.InnerException IsNot Nothing
                    ex = ex.InnerException
                End While
                Return New With {.data = Nothing, .error = ex.Message}
            End Try
        End Function

        <HttpPost>
        <ValidateInput(False)>
        Public Function SendReportEmail() As Object
            Try
                Dim qs = HttpContext.Current.Request.Form
                Dim iDBackupProfile = 0
                If Not String.IsNullOrEmpty(qs("iDBackupProfile")) Then
                    iDBackupProfile = qs("iDBackupProfile")
                End If
                Dim EmailTo = ""
                If Not String.IsNullOrEmpty(qs("EmailTo")) Then
                    EmailTo = qs("EmailTo")
                End If
                Dim Subject = ""
                If Not String.IsNullOrEmpty(qs("Subject")) Then
                    Subject = qs("Subject")
                End If
                Dim Body = ""
                If Not String.IsNullOrEmpty(qs("Body")) Then
                    Body = qs("Body")
                End If
                Dim MDate = Now
                If Not String.IsNullOrEmpty(qs("MDate")) Then
                    MDate = qs("MDate")
                End If
                Using mail As New MailMessage()
                    mail.From = New MailAddress("podpora@doctorum.cz", "DOCTORUM.CZ")
                    mail.To.Add(New MailAddress(EmailTo))
                    mail.Bcc.Add(New MailAddress("podpora@doctorum.cz"))
                    mail.Subject = Subject
                    mail.Body = Body
                    mail.IsBodyHtml = True

                    Dim smtp As SmtpClient = New SmtpClient()
                    smtp.Host = "smtp.forpsi.com"
                    smtp.EnableSsl = True

                    Dim networkCredential As NetworkCredential = New NetworkCredential("podpora@doctorum.cz", "Frolikova.321")
                    smtp.UseDefaultCredentials = True
                    smtp.Credentials = networkCredential
                    smtp.Port = 587
                    smtp.Send(mail)

                    Using db As New Data4995Entities
                        Dim iDUserUpravil = db.AG_tblUsers.FirstOrDefault(Function(e) e.UserLogin = User.Identity.Name).IDUser
                        db.AGsp_Do_IHistory(iDBackupProfile, iDUserUpravil, "Měsíční report", "Měsíční report")
                    End Using

                    Return New With {.message = Nothing}
                End Using
            Catch ex As Exception
                Return New With {.message = ex.Message}
            End Try
        End Function

        <HttpPost>
        Public Function Faktura()
            Try
                Dim q = HttpContext.Current.Request.Form
                Dim xel As XElement = <faltura></faltura>
                Dim inputXml As String = xel.ToString
                If Not String.IsNullOrEmpty(q("xml")) Then
                    Dim ele As XElement = XElement.Parse(q("xml"))
                    xel = ele
                    inputXml = ele.ToString
                End If

                Dim webKitPath = HttpContext.Current.Server.MapPath("~/App_Data/QtBinaries")
                Dim xsltPath = HttpContext.Current.Server.MapPath("~/App_Data/Faktura.xslt")
                Dim sbXslOutput As StringBuilder = New StringBuilder()

                Using xslWriter As XmlWriter = XmlWriter.Create(sbXslOutput, New XmlWriterSettings With {
                    .ConformanceLevel = ConformanceLevel.Fragment
                })
                    Dim transformer As XslCompiledTransform = New XslCompiledTransform()
                    transformer.Load(xsltPath)
                    Dim args As XsltArgumentList = New XsltArgumentList()
                    Dim doc As XmlDocument = New XmlDocument()
                    doc.LoadXml(inputXml)
                    transformer.Transform(doc, args, xslWriter)
                End Using
                Dim htmlText = sbXslOutput.ToString()

                Dim HtmlConverter As New HtmlToPdfConverter(HtmlRenderingEngine.WebKit)
                Dim settings As New WebKitConverterSettings()
                settings.WebKitPath = webKitPath
                settings.PdfPageSize = PdfPageSize.A4
                settings.WebKitViewPort = New Drawing.Size(800, 0)
                settings.Margin = New Graphics.PdfMargins With {.All = 10}
                settings.Orientation = PdfPageOrientation.Portrait
                settings.MediaType = MediaType.Print
                HtmlConverter.ConverterSettings = settings

                Dim document As PdfDocument = HtmlConverter.Convert(htmlText, String.Empty)
                Dim stream As MemoryStream = New MemoryStream()
                document.Save(stream)
                stream.Position = 0
                document.Close(True)

                Dim response As Http.HttpResponseMessage = New Http.HttpResponseMessage(HttpStatusCode.OK)
                Dim vs = xel.Descendants("vs").FirstOrDefault()
                response.Content = New Http.StreamContent(stream)
                response.Content.Headers.ContentType = New MediaTypeHeaderValue("aapplication/pdf")
                response.Content.Headers.ContentDisposition = New ContentDispositionHeaderValue("attachment")
                response.Content.Headers.ContentDisposition.FileName = IO.Path.GetFileName("Faktura-" & vs.Value & ".pdf")

                Return response
            Catch ex As Exception
                While ex.InnerException IsNot Nothing
                    ex = ex.InnerException
                End While
                Return New Http.HttpResponseMessage(HttpStatusCode.BadRequest) With {.Content = New Http.StringContent(ex.Message)}
            End Try
        End Function

        <HttpPost>
        Public Function Registration()
            Try
                Dim q = HttpContext.Current.Request.Form
                Dim id = 0
                If Not String.IsNullOrEmpty(q("id")) Then
                    id = CInt(q("id"))
                End If
                Dim email = ""
                If Not String.IsNullOrEmpty(q("email")) Then
                    email = q("email")
                End If
                Dim tel = ""
                If Not String.IsNullOrEmpty(q("tel")) Then
                    tel = q("tel")
                End If
                Dim name = ""
                If Not String.IsNullOrEmpty(q("name")) Then
                    name = q("name")
                End If

                Dim lL_LastLapse As New ObjectParameter("LL_LastLapse", GetType(Integer))
                Dim aktivacniKod As New ObjectParameter("AktivacniKod", GetType(String))
                Using db As New Data4995Entities
                    db.AGsp_Do_UBackupRegistracniUdaje(id, name, tel, email, aktivacniKod, lL_LastLapse)
                    If lL_LastLapse.Value > 0 Then
                        Return New With {.type = "error", .data = Nothing, .message = lastLapse(lL_LastLapse.Value)}
                    End If

                    Dim smtpClient = New SmtpClient("smtp.forpsi.com") With {
        .Port = 587,
        .Credentials = New NetworkCredential("noreply@doctorum.cz", "noreply.2019"),
        .EnableSsl = True
    }
                    Dim mailMessage = New MailMessage With {
        .From = New MailAddress("noreply@doctorum.cz", "SafeBerry"),
        .Subject = "🔐 Váš aktivační kód",
        .Body = <div>
                    <h3 style="text-align:center;">Zde je váš aktivační kód SafeBerry, <%= email %>!</h3>
                    <p style="text-align:center;">Pokračujte v registraci SafeBerry zadáním níže uvedeného kódu:</p>
                    <h1 style="text-align:center;"><%= aktivacniKod.Value %></h1>
                </div>.ToString,
        .IsBodyHtml = True
    }
                    mailMessage.[To].Add(email)
                    smtpClient.Send(mailMessage)

                    Return New With {.type = "success", .data = aktivacniKod.Value, .message = Nothing}
                End Using
            Catch ex As Exception
                While ex.InnerException IsNot Nothing
                    ex = ex.InnerException
                End While
                Return New With {.type = "error", .data = Nothing, .message = ex.Message}
            End Try
        End Function

        <HttpPost>
        Public Function Registration_G2()
            Try
                Dim q = HttpContext.Current.Request.Form
                Dim id = 0
                If Not String.IsNullOrEmpty(q("id")) Then
                    id = CInt(q("id"))
                End If
                Dim email = ""
                If Not String.IsNullOrEmpty(q("email")) Then
                    email = q("email")
                End If
                Dim tel = ""
                If Not String.IsNullOrEmpty(q("tel")) Then
                    tel = q("tel")
                End If
                Dim name = ""
                If Not String.IsNullOrEmpty(q("name")) Then
                    name = q("name")
                End If
                Dim street = ""
                If Not String.IsNullOrEmpty(q("street")) Then
                    street = q("street")
                End If
                Dim city = ""
                If Not String.IsNullOrEmpty(q("city")) Then
                    city = q("city")
                End If
                Dim psc = ""
                If Not String.IsNullOrEmpty(q("psc")) Then
                    psc = q("psc")
                End If
                Dim ico = ""
                If Not String.IsNullOrEmpty(q("ico")) Then
                    ico = q("ico")
                End If
                Dim dic = ""
                If Not String.IsNullOrEmpty(q("dic")) Then
                    dic = q("dic")
                End If
                Dim firma = ""
                If Not String.IsNullOrEmpty(q("firma")) Then
                    firma = q("firma")
                End If

                Dim lL_LastLapse As New ObjectParameter("LL_LastLapse", GetType(Integer))
                Dim aktivacniKod As New ObjectParameter("AktivacniKod", GetType(String))
                Using db As New Data4995Entities
                    db.AGsp_Do_UBackupRegistracniUdaje_G2(id, name, tel, email, ico, dic, firma, street, psc, city, aktivacniKod, lL_LastLapse)
                    If lL_LastLapse.Value > 0 Then
                        Return New With {.type = "error", .data = Nothing, .message = lastLapse(lL_LastLapse.Value)}
                    End If

                    If String.IsNullOrWhiteSpace(email) Then
                        Return New With {.type = "success", .data = Nothing, .message = Nothing}
                    End If

                    Dim smtpClient = New SmtpClient("smtp.forpsi.com") With {
        .Port = 587,
        .Credentials = New NetworkCredential("noreply@doctorum.cz", "noreply.2019"),
        .EnableSsl = True
    }
                    Dim mailMessage = New MailMessage With {
        .From = New MailAddress("noreply@doctorum.cz", "SafeBerry"),
        .Subject = "🔐 Váš ověřovací kód",
        .Body = <div>
                    <h3 style="text-align:center;">Zde je váš ověřovací kód SafeBerry, <%= email %>!</h3>
                    <p style="text-align:center;">Pokračujte v ověřování SafeBerry zadáním níže uvedeného kódu:</p>
                    <h1 style="text-align:center;"><%= aktivacniKod.Value %></h1>
                </div>.ToString,
        .IsBodyHtml = True
    }
                    mailMessage.[To].Add(email)
                    smtpClient.Send(mailMessage)

                    Return New With {.type = "success", .data = aktivacniKod.Value, .message = Nothing}
                End Using
            Catch ex As Exception
                While ex.InnerException IsNot Nothing
                    ex = ex.InnerException
                End While
                Return New With {.type = "error", .data = Nothing, .message = ex.Message}
            End Try
        End Function

        <HttpPost>
        Public Function Activate()
            Try
                Dim q = HttpContext.Current.Request.Form
                Dim id = 0
                If Not String.IsNullOrEmpty(q("id")) Then
                    id = CInt(q("id"))
                End If
                Dim code = ""
                If Not String.IsNullOrEmpty(q("code")) Then
                    code = q("code")
                End If

                Dim lL_LastLapse As New ObjectParameter("LL_LastLapse", GetType(Integer))
                Using db As New Data4995Entities
                    db.AGsp_Do_UBackupRegistracePotvrzeni(id, code, lL_LastLapse)
                    If lL_LastLapse.Value > 0 Then
                        If lL_LastLapse.Value = 118 Then
                            Return New With {.type = "success", .data = Nothing, .message = lastLapse(lL_LastLapse.Value)}
                        Else
                            Return New With {.type = "error", .data = Nothing, .message = lastLapse(lL_LastLapse.Value)}
                        End If
                    End If
                    Return New With {.type = "success", .data = Nothing, .message = Nothing}
                End Using
            Catch ex As Exception
                While ex.InnerException IsNot Nothing
                    ex = ex.InnerException
                End While
                Return New With {.type = "error", .data = Nothing, .message = ex.Message}
            End Try
        End Function

        <HttpGet>
        Public Function NahoditRestartovat(id As String, stav As Integer) As String
            Try
                Dim lL_LastLapse As New ObjectParameter("LL_LastLapse", GetType(Integer))
                Using db As New Data4995Entities
                    db.AGsp_Do_NahoditSignalR(id, stav, lL_LastLapse)
                    If lL_LastLapse.Value > 0 Then
                        Return lastLapse(lL_LastLapse.Value)
                    End If
                    Return Nothing
                End Using
            Catch ex As Exception
                Return Nothing
            End Try
        End Function

        <HttpGet>
        Public Sub GetTimes(id As String, caller As String)
            Try
                hub.Clients.All.GetTimes(id, caller)
            Catch ex As Exception
            End Try
        End Sub

        <HttpGet>
        Public Sub GetDeviceInformation(id As String, caller As String, win32 As String)
            Try
                hub.Clients.All.GetDeviceInformation(id, caller, win32)
            Catch ex As Exception
            End Try
        End Sub

        <HttpGet>
        Public Sub RemoteUpdate(id As String)
            Try
                hub.Clients.All.RemoteUpdate(id)
            Catch ex As Exception
            End Try
        End Sub

        <HttpGet>
        Public Sub SetTimes(id As String, times As String)
            Try
                hub.Clients.All.SetTimes(id, times)
            Catch ex As Exception
            End Try
        End Sub

        <HttpGet>
        Public Sub ClientMessage(caller As String, id As Integer, title As String, message As String, btn As Integer, icon As Integer)
            Try
                hub.Clients.All.ClientMessage(caller, id, 0, title, message, btn, icon)
            Catch ex As Exception
            End Try
        End Sub

        <HttpGet>
        Public Function ClientEmail(email As String, subject As String, body As String)
            Try
                Dim smtpClient = New SmtpClient("smtp.forpsi.com") With {
        .Port = 587,
        .Credentials = New NetworkCredential("noreply@doctorum.cz", "noreply.2019"),
        .EnableSsl = True
    }
                Dim mailMessage = New MailMessage With {
    .From = New MailAddress("noreply@doctorum.cz", "SafeBerry"),
    .Subject = subject,
    .Body = <p><%= body %></p>.ToString,
    .IsBodyHtml = True
}
                mailMessage.[To].Add(email)
                smtpClient.Send(mailMessage)

                Return Nothing
            Catch ex As Exception
                While ex.InnerException IsNot Nothing
                    ex = ex.InnerException
                End While
                Return ex.Message
            End Try
        End Function

        <HttpGet>
        Public Function DownloadSafeBerryInstall()
            Try
                Dim path = HttpContext.Current.Server.MapPath("/App_Data/SafeBerry/")
                Dim file As String = IO.Path.Combine(path, "SafeBerryInstall.exe")
                If IO.File.Exists(file) Then
                    Dim response As Http.HttpResponseMessage = New Http.HttpResponseMessage(HttpStatusCode.OK)
                    Dim fs = New FileStream(file, FileMode.Open)
                    response.Content = New Http.StreamContent(fs)
                    response.Content.Headers.ContentType = New MediaTypeHeaderValue("application/octet-stream")
                    response.Content.Headers.ContentDisposition = New ContentDispositionHeaderValue("attachment")
                    response.Content.Headers.ContentDisposition.FileName = IO.Path.GetFileName(file)

                    Try
                        Dim ip As String = HttpContext.Current.Request.ServerVariables("HTTP_X_FORWARDED_FOR")
                        If String.IsNullOrEmpty(ip) Then
                            ip = HttpContext.Current.Request.ServerVariables("REMOTE_ADDR")
                        End If
                        Dim ipinfoadr = New WebClient().DownloadString("http://ipinfo.io/" + ip)
                        Dim ipinfo = JsonConvert.DeserializeObject(Of IpInfo)(ipinfoadr)
                        Using db As New Data4995Entities
                            db.AGsp_Do_ISafeBerryDownload(ipinfo.Ip, ipinfo.Country, ipinfo.City)
                        End Using
                    Catch ex As Exception
                    End Try

                    Return response
                Else
                    Return Nothing
                End If
            Catch ex As Exception
                Return Nothing
            End Try
        End Function

        <HttpGet>
        Public Function DownloadSafeBerry()
            Try
                Dim path = HttpContext.Current.Server.MapPath("/App_Data/SafeBerry/")
                Dim latestversion = XDocument.Load(IO.Path.Combine(path, "latestversion.xml"))
                Dim version = latestversion.Root.<version>.Value
                Dim file As String = IO.Path.Combine(path & "\" & version, "SafeBerry.msi")
                If IO.File.Exists(file) Then
                    Dim response As Http.HttpResponseMessage = New Http.HttpResponseMessage(HttpStatusCode.OK)
                    Dim fs = New FileStream(file, FileMode.Open)
                    response.Content = New Http.StreamContent(fs)
                    response.Content.Headers.ContentType = New MediaTypeHeaderValue("application/octet-stream")
                    response.Content.Headers.ContentDisposition = New ContentDispositionHeaderValue("attachment")
                    response.Content.Headers.ContentDisposition.FileName = IO.Path.GetFileName(file)
                    Return response
                Else
                    Dim response As Http.HttpResponseMessage = New Http.HttpResponseMessage(HttpStatusCode.NotFound)
                    Return response
                End If
            Catch ex As Exception
                Dim response As Http.HttpResponseMessage = New Http.HttpResponseMessage(HttpStatusCode.InternalServerError)
                Return response
            End Try
        End Function

        <HttpGet>
        Public Function TeamViewerQS()
            Try
                Dim file = HttpContext.Current.Server.MapPath("/App_Data/SafeBerry/TeamViewerQS.exe")
                If IO.File.Exists(file) Then
                    Dim response As Http.HttpResponseMessage = New Http.HttpResponseMessage(HttpStatusCode.OK)
                    Dim fs = New FileStream(file, FileMode.Open)
                    response.Content = New Http.StreamContent(fs)
                    response.Content.Headers.ContentType = New MediaTypeHeaderValue("application/octet-stream")
                    response.Content.Headers.ContentDisposition = New ContentDispositionHeaderValue("attachment")
                    response.Content.Headers.ContentDisposition.FileName = IO.Path.GetFileName(file)
                    Return response
                Else
                    Dim response As Http.HttpResponseMessage = New Http.HttpResponseMessage(HttpStatusCode.NotFound)
                    Return response
                End If
            Catch ex As Exception
                Dim response As Http.HttpResponseMessage = New Http.HttpResponseMessage(HttpStatusCode.InternalServerError)
                Return response
            End Try
        End Function

        <HttpGet>
        Public Function DownloadFile(filename As String)
            Try
                Dim path = HttpContext.Current.Server.MapPath("/App_Data/SafeBerry/")
                Dim latestversion = XDocument.Load(IO.Path.Combine(path, "latestversion.xml"))
                Dim version = latestversion.Root.<version>.Value
                Dim file As String = IO.Path.Combine(IO.Path.Combine(path, version), filename)
                If IO.File.Exists(file) Then
                    Dim response As Http.HttpResponseMessage = New Http.HttpResponseMessage(HttpStatusCode.OK)
                    Dim fs = New FileStream(file, FileMode.Open)
                    response.Content = New Http.StreamContent(fs)
                    response.Content.Headers.ContentType = New MediaTypeHeaderValue("application/octet-stream")
                    response.Content.Headers.ContentDisposition = New ContentDispositionHeaderValue("attachment")
                    response.Content.Headers.ContentDisposition.FileName = IO.Path.GetFileName(file)
                    Return response
                Else
                    Dim response As Http.HttpResponseMessage = New Http.HttpResponseMessage(HttpStatusCode.NotFound)
                    Return response
                End If
            Catch ex As Exception
                Dim response As Http.HttpResponseMessage = New Http.HttpResponseMessage(HttpStatusCode.InternalServerError)
                Return response
            End Try
        End Function

        <HttpGet>
        Public Function DownloadUpdateFiles(fileName As String)
            Try
                Dim path = HttpContext.Current.Server.MapPath("/App_Data/SafeBerry/")
                Dim latestversion = XDocument.Load(IO.Path.Combine(path, "latestversion.xml"))
                Dim version = latestversion.Root.<version>.Value
                Dim file As String = IO.Path.Combine(path & "\" & version, "Update\" & fileName)
                If IO.File.Exists(file) Then
                    Dim response As Http.HttpResponseMessage = New Http.HttpResponseMessage(HttpStatusCode.OK)
                    Dim fs = New FileStream(file, FileMode.Open)
                    response.Content = New Http.StreamContent(fs)
                    response.Content.Headers.ContentType = New MediaTypeHeaderValue("application/octet-stream")
                    response.Content.Headers.ContentDisposition = New ContentDispositionHeaderValue("attachment")
                    response.Content.Headers.ContentDisposition.FileName = IO.Path.GetFileName(file)
                    Return response
                Else
                    Return Nothing
                End If
            Catch ex As Exception
                Return Nothing
            End Try
        End Function

        <HttpGet>
        Public Function DownloadPCDOKTOR()
            Try
                Dim path = HttpContext.Current.Server.MapPath("/App_Data/SafeBerry/PC DOKTOR se zálohou.exe")
                If IO.File.Exists(path) Then
                    Dim response As Http.HttpResponseMessage = New Http.HttpResponseMessage(HttpStatusCode.OK)
                    Dim fs = New FileStream(path, FileMode.Open)
                    response.Content = New Http.StreamContent(fs)
                    response.Content.Headers.ContentType = New MediaTypeHeaderValue("application/octet-stream")
                    response.Content.Headers.ContentDisposition = New ContentDispositionHeaderValue("attachment")
                    response.Content.Headers.ContentDisposition.FileName = IO.Path.GetFileName(path)
                    Return response
                Else
                    Return Nothing
                End If
            Catch ex As Exception
                Return Nothing
            End Try
        End Function

        <HttpGet>
        Public Function DownloadPCDENT()
            Try
                Dim path = HttpContext.Current.Server.MapPath("/App_Data/SafeBerry/PC DENT se zálohou.exe")
                If IO.File.Exists(path) Then
                    Dim response As Http.HttpResponseMessage = New Http.HttpResponseMessage(HttpStatusCode.OK)
                    Dim fs = New FileStream(path, FileMode.Open)
                    response.Content = New Http.StreamContent(fs)
                    response.Content.Headers.ContentType = New MediaTypeHeaderValue("application/octet-stream")
                    response.Content.Headers.ContentDisposition = New ContentDispositionHeaderValue("attachment")
                    response.Content.Headers.ContentDisposition.FileName = IO.Path.GetFileName(path)
                    Return response
                Else
                    Return Nothing
                End If
            Catch ex As Exception
                Return Nothing
            End Try
        End Function

        <HttpGet>
        Public Function DownloadAMICUS()
            Try
                Dim path = HttpContext.Current.Server.MapPath("/App_Data/SafeBerry/AMICUS se zálohou.exe")
                If IO.File.Exists(path) Then
                    Dim response As Http.HttpResponseMessage = New Http.HttpResponseMessage(HttpStatusCode.OK)
                    Dim fs = New FileStream(path, FileMode.Open)
                    response.Content = New Http.StreamContent(fs)
                    response.Content.Headers.ContentType = New MediaTypeHeaderValue("application/octet-stream")
                    response.Content.Headers.ContentDisposition = New ContentDispositionHeaderValue("attachment")
                    response.Content.Headers.ContentDisposition.FileName = IO.Path.GetFileName(path)
                    Return response
                Else
                    Return Nothing
                End If
            Catch ex As Exception
                Return Nothing
            End Try
        End Function

        <HttpPost>
        <ValidateInput(False)>
        Public Overridable Function ClientSettings()
            Try
                Dim q = HttpContext.Current.Request.Form
                Dim iDBackupProfile = ""
                If Not String.IsNullOrEmpty(q("iDBackupProfile")) Then
                    iDBackupProfile = q("iDBackupProfile")
                End If
                Dim updatovat = False
                If Not String.IsNullOrEmpty(q("updatovat")) Then
                    updatovat = CBool(q("updatovat"))
                End If
                Dim backupTimeConfig = ""
                If Not String.IsNullOrEmpty(q("backupTimeConfig")) Then
                    backupTimeConfig = q("backupTimeConfig")
                End If
                Dim lL_LastLapse As New ObjectParameter("LL_LastLapse", GetType(Integer))
                Dim backupTimeConfigOUT As New ObjectParameter("BackupTimeConfigOUT", GetType(String))
                Using db As New Data4995Entities
                    db.AGsp_Do_UBackupTimeConfig(iDBackupProfile, updatovat, backupTimeConfig, backupTimeConfigOUT, lL_LastLapse)
                    If backupTimeConfigOUT.Value IsNot Nothing Then
                        Dim xml = backupTimeConfigOUT.Value
                        Return XElement.Parse(xml)
                    Else
                        Return Nothing
                    End If
                End Using
            Catch ex As Exception
                While ex.InnerException IsNot Nothing
                    ex = ex.InnerException
                End While
                Return Nothing
            End Try
        End Function

        <HttpPost>
        Public Sub UploadConfig()
            Try
                Dim q = HttpContext.Current.Request.Form
                Dim caller = Nothing
                If Not String.IsNullOrEmpty(q("caller")) Then
                    caller = q("caller")
                End If
                Dim iDKlienta = Nothing
                If Not String.IsNullOrEmpty(q("iDKlienta")) Then
                    iDKlienta = q("iDKlienta")
                End If

                Dim file As HttpPostedFile = HttpContext.Current.Request.Files("config")
                If file IsNot Nothing Then
                    Dim name As String = iDKlienta & "\" & file.FileName
                    Dim path = IO.Path.Combine(HttpContext.Current.Server.MapPath("~/Backups"), name)
                    If IO.File.Exists(path) Then
                        IO.File.Delete(path)
                    Else
                        Dim dir = IO.Path.GetDirectoryName(path)
                        IO.Directory.CreateDirectory(dir)
                    End If
                    file.SaveAs(path)
                    Dim xml = System.IO.File.ReadAllText(path)
                    hub.Clients.All.uploadConfig(xml, caller)
                Else
                    hub.Clients.All.uploadConfig(Nothing, caller)
                End If
            Catch ex As Exception
            End Try
        End Sub

        <HttpGet>
        Public Overridable Function AccInfo() As Object
            Try
                Dim q = HttpContext.Current.Request.QueryString

                Dim iDBackupProfile = 0
                If Not String.IsNullOrEmpty(q("iDBackupProfile")) Then
                    iDBackupProfile = q("iDBackupProfile")
                End If

                Dim appID As Guid = Nothing
                If Not String.IsNullOrEmpty(q("appID")) Then
                    appID = Guid.Parse(q("appID"))
                End If

                Dim iDProcesoru = ""
                If Not String.IsNullOrEmpty(q("iDProcesoru")) Then
                    iDProcesoru = q("iDProcesoru")
                End If

                Dim iDMACAdresa = ""
                If Not String.IsNullOrEmpty(q("iDMACAdresa")) Then
                    iDMACAdresa = q("iDMACAdresa")
                End If

                Dim userName = ""
                If Not String.IsNullOrEmpty(q("userName")) Then
                    userName = q("userName")
                End If

                Dim verzeSluzby = "1.0.0.0."
                If Not String.IsNullOrEmpty(q("verzeSluzby")) Then
                    verzeSluzby = q("verzeSluzby")
                End If

                Dim lL_LastLapse As New ObjectParameter("LL_LastLapse", GetType(Integer))
                Dim iDKlienta As New ObjectParameter("IDKlienta", GetType(Integer))
                Dim iDBackupProfileOUT As New ObjectParameter("IDBackupProfileOUT", GetType(Integer))
                Dim rr_StavBackupProfile As New ObjectParameter("rr_StavBackupProfile", GetType(Integer))
                Dim rr_StavBackupProfileText As New ObjectParameter("rr_StavBackupProfileText", GetType(String))
                Dim datumExpirace As New ObjectParameter("DatumExpirace", GetType(DateTime))
                Dim backupTimeConfig As New ObjectParameter("BackupTimeConfig", GetType(String))
                Dim appID_OUT As New ObjectParameter("AppID_OUT", GetType(Guid))

                Using db As New Data4995Entities
                    db.AGsp_Get_DoctorumBackupConfig_G2(iDBackupProfile, appID, iDProcesoru, iDMACAdresa, userName, verzeSluzby, iDKlienta, iDBackupProfileOUT, rr_StavBackupProfile, rr_StavBackupProfileText, datumExpirace, backupTimeConfig, appID_OUT, lL_LastLapse)

                    Dim path = HttpContext.Current.Server.MapPath("/App_Data/SafeBerry/")
                    Dim latestversion = XDocument.Load(IO.Path.Combine(path, "latestversion.xml"))
                    Return <d>
                               <i><%= iDBackupProfileOUT.Value %></i>
                               <uid><%= appID_OUT.Value %></uid>
                               <ii><%= iDKlienta.Value %></ii>
                               <si><%= rr_StavBackupProfile.Value %></si>
                               <st><%= rr_StavBackupProfileText.Value %></st>
                               <d><%= CDate(datumExpirace.Value).ToString("yyyy-MM-ddTHH:mm:ss") %></d>
                               <ll><%= lL_LastLapse.Value %></ll>
                               <v><%= latestversion.Root.<version>.Value %></v>
                           </d>
                End Using
            Catch ex As Exception
                While ex.InnerException IsNot Nothing
                    ex = ex.InnerException
                End While
                Return Nothing
            End Try
        End Function

        <HttpGet>
        Public Overridable Function AccInfo_G3() As Object
            Try
                Dim q = HttpContext.Current.Request.QueryString

                Dim iDBackupProfile = 0
                If Not String.IsNullOrEmpty(q("iDBackupProfile")) Then
                    iDBackupProfile = q("iDBackupProfile")
                End If

                Dim appID As Guid = Nothing
                If Not String.IsNullOrEmpty(q("appID")) Then
                    appID = Guid.Parse(q("appID"))
                End If

                Dim iDProcesoru = ""
                If Not String.IsNullOrEmpty(q("iDProcesoru")) Then
                    iDProcesoru = q("iDProcesoru")
                End If

                Dim iDMACAdresa = ""
                If Not String.IsNullOrEmpty(q("iDMACAdresa")) Then
                    iDMACAdresa = q("iDMACAdresa")
                End If

                Dim userName = ""
                If Not String.IsNullOrEmpty(q("userName")) Then
                    userName = q("userName")
                End If

                Dim verzeSluzby = "1.0.0.0."
                If Not String.IsNullOrEmpty(q("verzeSluzby")) Then
                    verzeSluzby = q("verzeSluzby")
                End If

                Dim lL_LastLapse As New ObjectParameter("LL_LastLapse", GetType(Integer))
                Dim iDKlienta As New ObjectParameter("IDKlienta", GetType(Integer))
                Dim iDBackupProfileOUT As New ObjectParameter("IDBackupProfileOUT", GetType(Integer))
                Dim rr_StavBackupProfile As New ObjectParameter("rr_StavBackupProfile", GetType(Integer))
                Dim rr_StavBackupProfileText As New ObjectParameter("rr_StavBackupProfileText", GetType(String))
                Dim datumExpirace As New ObjectParameter("DatumExpirace", GetType(DateTime))
                Dim backupTimeConfig As New ObjectParameter("BackupTimeConfig", GetType(String))
                Dim appID_OUT As New ObjectParameter("AppID_OUT", GetType(Guid))
                Dim nahoditSignalR As New ObjectParameter("NahoditSignalR", GetType(Integer))

                Using db As New Data4995Entities
                    db.AGsp_Get_DoctorumBackupConfig_G3(iDBackupProfile, appID, iDProcesoru, iDMACAdresa, userName, verzeSluzby, iDKlienta, iDBackupProfileOUT, rr_StavBackupProfile, rr_StavBackupProfileText, datumExpirace, backupTimeConfig, appID_OUT, nahoditSignalR, lL_LastLapse)

                    Dim path = HttpContext.Current.Server.MapPath("/App_Data/SafeBerry/")
                    Dim latestversion = XDocument.Load(IO.Path.Combine(path, "latestversion.xml"))
                    Return <d>
                               <i><%= iDBackupProfileOUT.Value %></i>
                               <uid><%= appID_OUT.Value %></uid>
                               <ii><%= iDKlienta.Value %></ii>
                               <si><%= rr_StavBackupProfile.Value %></si>
                               <st><%= rr_StavBackupProfileText.Value %></st>
                               <d><%= CDate(datumExpirace.Value).ToString("yyyy-MM-ddTHH:mm:ss") %></d>
                               <ll><%= lL_LastLapse.Value %></ll>
                               <v><%= latestversion.Root.<version>.Value %></v>
                               <r><%= nahoditSignalR.Value %></r>
                           </d>
                End Using

            Catch ex As Exception
                While ex.InnerException IsNot Nothing
                    ex = ex.InnerException
                End While
                Return Nothing
            End Try
        End Function

        <HttpGet>
        Public Overridable Function AccInfo_G4() As Object
            Try
                Dim q = HttpContext.Current.Request.QueryString

                Dim iDBackupProfile = 0
                If Not String.IsNullOrEmpty(q("iDBackupProfile")) Then
                    iDBackupProfile = q("iDBackupProfile")
                End If

                Dim appID As Guid = Nothing
                If Not String.IsNullOrEmpty(q("appID")) Then
                    appID = Guid.Parse(q("appID"))
                End If

                Dim iDProcesoru = ""
                If Not String.IsNullOrEmpty(q("iDProcesoru")) Then
                    iDProcesoru = q("iDProcesoru")
                End If

                Dim iDMACAdresa = ""
                If Not String.IsNullOrEmpty(q("iDMACAdresa")) Then
                    iDMACAdresa = q("iDMACAdresa")
                End If

                Dim userName = ""
                If Not String.IsNullOrEmpty(q("userName")) Then
                    userName = q("userName")
                End If

                Dim verzeSluzby = "1.0.0.0."
                If Not String.IsNullOrEmpty(q("verzeSluzby")) Then
                    verzeSluzby = q("verzeSluzby")
                End If

                Dim lL_LastLapse As New ObjectParameter("LL_LastLapse", GetType(Integer))
                Dim sBvRezimuServer As New ObjectParameter("SBvRezimuServer", GetType(Boolean))
                Dim iDKlienta As New ObjectParameter("IDKlienta", GetType(Integer))
                Dim iDBackupProfileOUT As New ObjectParameter("IDBackupProfileOUT", GetType(Integer))
                Dim rr_StavBackupProfile As New ObjectParameter("rr_StavBackupProfile", GetType(Integer))
                Dim rr_StavBackupProfileText As New ObjectParameter("rr_StavBackupProfileText", GetType(String))
                Dim datumExpirace As New ObjectParameter("DatumExpirace", GetType(DateTime))
                Dim backupTimeConfig As New ObjectParameter("BackupTimeConfig", GetType(String))
                Dim appID_OUT As New ObjectParameter("AppID_OUT", GetType(Guid))
                Dim nahoditSignalR As New ObjectParameter("NahoditSignalR", GetType(Integer))

                Using db As New Data4995Entities
                    db.AGsp_Get_DoctorumBackupConfig_G4(iDBackupProfile, appID, iDProcesoru, iDMACAdresa, userName, verzeSluzby, iDKlienta, iDBackupProfileOUT, rr_StavBackupProfile, rr_StavBackupProfileText, datumExpirace, backupTimeConfig, appID_OUT, nahoditSignalR, sBvRezimuServer, lL_LastLapse)

                    Dim path = HttpContext.Current.Server.MapPath("/App_Data/SafeBerry/")
                    Dim latestversion = XDocument.Load(IO.Path.Combine(path, "latestversion.xml"))
                    Return <d>
                               <i><%= iDBackupProfileOUT.Value %></i>
                               <uid><%= appID_OUT.Value %></uid>
                               <ii><%= iDKlienta.Value %></ii>
                               <si><%= rr_StavBackupProfile.Value %></si>
                               <st><%= rr_StavBackupProfileText.Value %></st>
                               <d><%= CDate(datumExpirace.Value).ToString("yyyy-MM-ddTHH:mm:ss") %></d>
                               <ll><%= lL_LastLapse.Value %></ll>
                               <v><%= latestversion.Root.<version>.Value %></v>
                               <r><%= nahoditSignalR.Value %></r>
                               <t><%= sBvRezimuServer.Value %></t>
                           </d>
                End Using
            Catch ex As Exception
                While ex.InnerException IsNot Nothing
                    ex = ex.InnerException
                End While
                Return Nothing
            End Try
        End Function

        <HttpPost>
        <ValidateInput(False)>
        Public Overridable Function BackupInfo()
            Try
                Dim q = HttpContext.Current.Request.Form
                Dim userName = ""
                If Not String.IsNullOrEmpty(q("userName")) Then
                    userName = q("userName")
                End If
                Dim appID As Guid = Nothing
                If Not String.IsNullOrEmpty(q("appID")) Then
                    appID = Guid.Parse(q("appID"))
                End If
                Dim path = ""
                If Not String.IsNullOrEmpty(q("path")) Then
                    path = q("path")
                End If
                Dim config = ""
                Dim size As Int64 = 0
                Dim lL_LastLapse As New ObjectParameter("LL_LastLapse", GetType(Integer))

                Dim file As HttpPostedFile = HttpContext.Current.Request.Files("config")
                If file IsNot Nothing Then
                    Using reader As New StreamReader(file.InputStream)
                        config = reader.ReadToEnd
                    End Using
                End If

                Dim fi As New IO.FileInfo(HttpContext.Current.Server.MapPath(path))
                If IO.File.Exists(fi.FullName) Then
                    size = fi.Length
                End If

                Using db As New Data4995Entities
                    Dim item = db.AGsp_Get_BackupServerovaSlozka_G2(appID).FirstOrDefault
                    If item IsNot Nothing Then
                        DeleteOldBackups(item.IDBackupProfile)
                        Try
                            db.AGsp_Do_INovyBackup_G2(item.IDBackupProfile, path, 1, size, config, lL_LastLapse)
                        Catch ex As Exception
                            db.AGsp_Do_INovyBackup_G2(item.IDBackupProfile, path, 2, size, config, lL_LastLapse)
                        End Try
                    End If
                End Using

                Return <response>
                           <type>success</type>
                           <data></data>
                           <message></message>
                       </response>

            Catch ex As Exception
                While ex.InnerException IsNot Nothing
                    ex = ex.InnerException
                End While
                Return <response>
                           <type>error</type>
                           <data></data>
                           <message><%= ex.Message %></message>
                       </response>
            End Try
        End Function

        Public Sub DeleteOldBackups(IDBackupProfile As Integer)
            Try
                Dim lL_LastLapse As New ObjectParameter("LL_LastLapse", GetType(Integer))
                Using db As New Data4995Entities
                    Dim data = db.AGsp_Get_BackupZalohyJednohoProfilu(IDBackupProfile).OrderByDescending(Function(e) e.IDBackup).Skip(5).ToList
                    For Each item In data
                        Dim path = HttpContext.Current.Server.MapPath(item.BackupFileName)
                        If IO.File.Exists(path) Then
                            Dim df As DateTime = IO.File.GetCreationTime(path)
                            If df < Now.AddDays(-7) Then
                                IO.File.Delete(path)
                                db.AGsp_Do_UBackupStav(item.IDBackup, 4, lL_LastLapse)
                            Else
                                db.AGsp_Do_UBackupStav(item.IDBackup, 1, lL_LastLapse)
                            End If
                        Else
                            db.AGsp_Do_UBackupStav(item.IDBackup, 4, lL_LastLapse)
                        End If
                    Next
                End Using
            Catch ex As Exception
            End Try
        End Sub

        <HttpPost>
        Public Overridable Function UploadChunk()
            Try
                Dim qs = HttpContext.Current.Request.Form
                Dim hash = ""
                If Not String.IsNullOrEmpty(qs("hash")) Then
                    hash = qs("hash")
                End If
                Dim size As Int64 = 0
                If Not String.IsNullOrEmpty(qs("size")) Then
                    size = qs("size")
                End If
                Dim path = ""
                If Not String.IsNullOrEmpty(qs("path")) Then
                    path = qs("path")
                End If
                Dim id = 0
                If Not String.IsNullOrEmpty(qs("id")) Then
                    id = qs("id")
                End If
                Dim text = ""
                If Not String.IsNullOrEmpty(qs("text")) Then
                    text = qs("text")
                End If

                If id > 0 Then
                    Dim lL_LastLapse As New ObjectParameter("LL_LastLapse", GetType(Integer))
                    Using db As New Data4995Entities
                        db.AGsp_Do_UDoctorumBackupAktualniBalicek(id, text, lL_LastLapse)
                    End Using
                End If

                path = HttpContext.Current.Server.MapPath(path)
                Dim fi As New IO.FileInfo(path)
                If Not IO.Directory.Exists(fi.DirectoryName) Then
                    IO.Directory.CreateDirectory(fi.DirectoryName)
                End If

                Dim file As HttpPostedFile = HttpContext.Current.Request.Files("chunk")
                If file IsNot Nothing Then
                    Using stream As New IO.FileStream(path, FileMode.Append)
                        If stream.Length <= size Then
                            Using binaryReader As New BinaryReader(file.InputStream)
                                Dim bytes = binaryReader.ReadBytes(binaryReader.BaseStream.Length)
                                stream.Write(bytes, 0, bytes.Length)
                                binaryReader.Close()
                            End Using
                        End If
                    End Using
                End If

                Return <response>
                           <type>success</type>
                           <data></data>
                           <message></message>
                       </response>
            Catch ex As Exception
                While ex.InnerException IsNot Nothing
                    ex = ex.InnerException
                End While
                Return <response>
                           <type>error</type>
                           <data></data>
                           <message><%= ex.Message %></message>
                       </response>
            End Try
        End Function

        <HttpPost>
        Public Overridable Function UploadOther()
            Dim filename = Nothing
            Try
                Dim qs = HttpContext.Current.Request.Form
                Dim size As Int64 = 0
                If Not String.IsNullOrEmpty(qs("size")) Then
                    size = qs("size")
                End If
                Dim sn = "none"
                If Not String.IsNullOrEmpty(qs("sn")) Then
                    sn = qs("sn")
                End If
                Dim id = 0
                If Not String.IsNullOrEmpty(qs("id")) Then
                    id = qs("id")
                End If

                Dim folder = HttpContext.Current.Server.MapPath("/Backups/" & id & "/Other/" & sn)
                If Not IO.Directory.Exists(folder) Then
                    IO.Directory.CreateDirectory(folder)
                End If

                Dim file As HttpPostedFile = HttpContext.Current.Request.Files("chunk")
                If file IsNot Nothing Then
                    filename = IO.Path.Combine(folder, file.FileName)
                    Using stream As New IO.FileStream(filename, FileMode.Append)
                        If stream.Length <= size Then
                            Using binaryReader As New BinaryReader(file.InputStream)
                                Dim bytes = binaryReader.ReadBytes(binaryReader.BaseStream.Length)
                                stream.Write(bytes, 0, bytes.Length)
                                binaryReader.Close()
                            End Using
                        End If
                    End Using
                Else
                    Throw New ArgumentException("File Not Found")
                End If

                Return StatusCode(200)
            Catch ex As Exception
                While ex.InnerException IsNot Nothing
                    ex = ex.InnerException
                End While
                If IO.File.Exists(filename) Then
                    IO.File.Delete(filename)
                End If
                Return StatusCode(500)
            End Try
        End Function

        <HttpPost>
        Public Sub UploadOtherXml()
            Dim filename = Nothing
            Try
                Dim q = HttpContext.Current.Request.Form
                Dim id = Nothing
                If Not String.IsNullOrEmpty(q("id")) Then
                    id = q("id")
                End If
                Dim sn = "none"
                If Not String.IsNullOrEmpty(q("sn")) Then
                    sn = q("sn")
                End If

                Dim folder = HttpContext.Current.Server.MapPath("/Backups/" & id & "/Other/" & sn)
                If Not IO.Directory.Exists(folder) Then
                    IO.Directory.CreateDirectory(folder)
                End If

                Dim file As HttpPostedFile = HttpContext.Current.Request.Files("other")
                If file IsNot Nothing Then
                    filename = IO.Path.Combine(folder, file.FileName)
                    If IO.File.Exists(filename) Then
                        IO.File.Delete(filename)
                    End If
                    file.SaveAs(filename)
                Else
                    Throw New ArgumentException("File Not Found")
                End If
            Catch ex As Exception
                If IO.File.Exists(filename) Then
                    Dim fi As New IO.FileInfo(filename)
                    Dim ext = fi.Extension
                    Dim encfile = IO.Path.Combine(fi.DirectoryName, fi.Name.Replace(ext, ".enc"))
                    If IO.File.Exists(encfile) Then
                        IO.File.Delete(encfile)
                    End If
                    IO.File.Delete(filename)
                End If
            End Try
        End Sub

        Private Function FileSize(len As Long)
            Dim sizes As String() = {"B", "KB", "MB", "GB", "TB"}
            Dim order As Integer = 0

            While len >= 1024 AndAlso order < sizes.Length - 1
                order += 1
                len = len / 1024
            End While

            Return String.Format("{0:0.##} {1}", len, sizes(order))
        End Function

        <HttpGet>
        Public Function BackupsList(id As Integer)
            Try
                Using db As New Data4995Entities
                    Dim data = db.AGsp_Get_BackupZalohyJednohoProfilu_G2(id).Where(Function(e) e.rr_BackupUspesnost = 1).ToList
                    Dim xml As XElement = New XElement("CloudBackups", data.Select(Function(e) New XElement("item", New List(Of XElement) From {New XElement("BackupDate", e.BackupDate.ToString("dd.MM.yyyy HH:mm")), New XElement("BackupFileName", e.BackupFileName), New XElement("BackupSize", FileSize(e.VelikostZalohy))})))
                    Return xml
                End Using
            Catch ex As Exception
                While ex.InnerException IsNot Nothing
                    ex = ex.InnerException
                End While
                Return Nothing
            End Try
        End Function

        <HttpGet>
        Public Function DownloadBackup()
            Try
                Dim qs = HttpContext.Current.Request.QueryString
                Dim path = "~/App_Data"
                If Not String.IsNullOrEmpty(qs("path")) Then
                    path = qs("path")
                End If
                Dim sp As String = HttpContext.Current.Server.MapPath(path)

                If IO.File.Exists(sp) Then
                    Dim response As Http.HttpResponseMessage = New Http.HttpResponseMessage(HttpStatusCode.OK)
                    Dim fs = New FileStream(sp, FileMode.Open)
                    response.Content = New Http.StreamContent(fs)
                    response.Content.Headers.ContentType = New MediaTypeHeaderValue("application/octet-stream")
                    response.Content.Headers.ContentDisposition = New ContentDispositionHeaderValue("attachment")
                    response.Content.Headers.ContentDisposition.FileName = IO.Path.GetFileName(sp)
                    Return response
                Else
                    Return Nothing
                End If

            Catch ex As Exception
                Return Nothing
            End Try
        End Function

        <HttpPost>
        Public Overridable Function UserMessage()
            Try
                Dim qs = HttpContext.Current.Request.Form
                Dim libi As Boolean = False
                If Not String.IsNullOrEmpty(qs("libi")) Then
                    libi = qs("libi")
                End If
                Dim vadi As Boolean = False
                If Not String.IsNullOrEmpty(qs("vadi")) Then
                    vadi = qs("vadi")
                End If
                Dim zprava = ""
                If Not String.IsNullOrEmpty(qs("zprava")) Then
                    zprava = qs("zprava")
                End If
                Dim emailNaOdpoved = ""
                If Not String.IsNullOrEmpty(qs("emailNaOdpoved")) Then
                    emailNaOdpoved = qs("emailNaOdpoved")
                End If

                Dim lL_LastLapse As New ObjectParameter("LL_LastLapse", GetType(Integer))
                Dim newID As New ObjectParameter("NewID", GetType(Integer))
                Using db As New Data4995Entities
                    db.AGsp_Do_ISafeBerryZpravaOdKlienta(libi, vadi, zprava, emailNaOdpoved, newID, lL_LastLapse)
                End Using

                Return Nothing
            Catch ex As Exception
                Return Nothing
            End Try
        End Function

        <HttpGet>
        Public Function ClientMessage() As Object
            Dim qs = HttpContext.Current.Request.QueryString
            Dim msgid As Integer = 0
            If Not String.IsNullOrEmpty(qs("msgid")) Then
                msgid = qs("msgid")
            End If
            Dim idklienta As String = Nothing
            If Not String.IsNullOrEmpty(qs("idklienta")) Then
                idklienta = qs("idklienta")
            End If
            Dim title As String = Nothing
            If Not String.IsNullOrEmpty(qs("title")) Then
                title = qs("title")
            End If
            Dim message As String = Nothing
            If Not String.IsNullOrEmpty(qs("message")) Then
                message = qs("message")
            End If
            Dim btn As Integer = 0
            If Not String.IsNullOrEmpty(qs("btn")) Then
                btn = qs("btn")
            End If
            Dim icon As Integer = 0
            If Not String.IsNullOrEmpty(qs("icon")) Then
                icon = qs("icon")
            End If

            hub.Clients.All.ClientMessage(msgid, idklienta, title, message, btn, icon)

            Return True
        End Function

        <HttpPost>
        Public Function ClientMessageCallback() As Object
            Dim qs = HttpContext.Current.Request.Form
            Dim msgid As Integer = 0
            If Not String.IsNullOrEmpty(qs("msgid")) Then
                msgid = qs("msgid")
            End If
            Dim iDKlienta As String = Nothing
            If Not String.IsNullOrEmpty(qs("iDKlienta")) Then
                iDKlienta = qs("iDKlienta")
            End If
            Dim result As Boolean = False
            If Not String.IsNullOrEmpty(qs("result")) Then
                result = qs("result")
            End If



            Return True
        End Function

    End Class
End Namespace